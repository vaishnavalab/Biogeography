...
#samdf <- read.table("Biogeography_Mappingfile.txt")
...
Kellys_Fecal <- Kellys_Kvon_ABX_Experiment %>%
  subset_samples(Organ == "Fecal")
Kellys_Fecal
...
Jax_Homeostasis <- physeq1_trim %>%
  subset_samples(Description == "Jax_Homeostasis")
Jax_Homeostasis
...
##Jackson Rarefied Diversity Table
Analysis<-Jax_Homeostasis
set.seed(42)
rarefied_analysis = rarefy_even_depth(Analysis)
rarefied_analysis<-Analysis
p = plot_richness(rarefied_analysis, x="Location", color="MouseID", measures=c("Observed"))
p + geom_point(size=5, alpha=0.7) + theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank()) 
rarefied_values<-p$data
rarefied_values
write.csv(rarefied_values, file = "rarefied_diversity_values_Jax.csv")
